...
class AccountInvoice(models.Model):
    ...
    def purchase_order_change(self):
        ...
        if vendor_ref and (
            not self.reference
            or (
                vendor_ref + ", " not in self.reference
                and not self.reference.endswith(vendor_ref)
            )
        ):
            self.reference = (
                ", ".join([self.reference, vendor_ref])
                if self.reference
                else vendor_ref
            )
        ...
        for line in self.purchase_id.order_line - self.invoice_line_ids.mapped(
            ...
            line_match = self.invoice_line_ids.filtered(
                lambda r: r.quantity == line.product_qty
    ...
    def _compute_purchase_order_ids(self):
        for record in self:
            record.purchase_order_ids = record.invoice_line_ids.mapped(
                "purchase_id"
            ).ids
