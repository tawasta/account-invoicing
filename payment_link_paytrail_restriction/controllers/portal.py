...
class PaytrailPaymentPortal(PaymentPortal):
    ...
    def payment_pay(
        ...
        response = super(PaytrailPaymentPortal, self).payment_pay(
            ...
            partner_id=partner_id, company_id=company_id, access_token=access_token, **kwargs
        )

        payment_methods_sudo = response.qcontext.get('payment_methods_sudo', []).filtered(
            lambda method: method.code == 'paytrail'
        )
